...
class AudioEditorMod(loader.Module):
    ...
    async def slowscmd(self, m):
        """.slows <factor> <reply to audio>
            SlowDown"""
        args = utils.get_args_raw(m)
        if not args:
            lvl = 0.5
        elif re.match(r'^\d+(\.\d+)?$', args) and (0.1 < float(args) < 0.9):
            lvl = float(args)
        audio = await get_audio(self, m, "SlowDown")
        if not audio:
            return
        s2 = audio.audio._spawn(audio.audio.raw_data, overrides={
            "frame_rate": int(audio.audio.frame_rate * float(args))})
    ...
    async def fastscmd(self, m):
        """.fasts <factor> <reply to audio>
        SpeedUp"""
        args = utils.get_args_raw(m)
        if not args:
            lvl = 2.0
        elif re.match(r'^\d+(\.\d+)?$', args) and (1.0 < float(args) < 1.5):
            lvl = float(args)
